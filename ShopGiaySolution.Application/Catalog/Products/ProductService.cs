...
using ShopGiaySolution.Utilities.Exceptions;
using System;
...
namespace ShopGiaySolution.Application.Catalog.Products
{
    public class ProductService : IProductService
    {
        ...
        public ProductService(ShopGiaydbContext context, IStorageService storageService)
        {
            _context= context;
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            if (product==null) throw new ShopGiayException($"Cannot find a product:{productId}");
            var imgages =  _context.ProductImages.Where(i => i.ProductId == productId);

            foreach (var image  in imgages)
            {
                await _storagesService.DeleteFileAsync(image.ImagePath!);
            }
        ...
        public async Task<PagedResult<ProductVm>> GetAllPaging(GetManagerProductPagingRequest request)
        {
        ...
        public async Task<ProductImageViewModel> GetImageById(int imageId)
        {
            ...
            if (image == null)
                throw new ShopGiayException($"Cannot find an image with id {imageId}");
        ...
        public async Task<int> Update( ProductUpdateRequest request)
        {
            ...
            if (product==null || productTranslation==null) throw new ShopGiayException($"Cannot find a product with id :{request.Id}");
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null ) throw new ShopGiayException($"Cannot find a product with id :{productId}");
        ...
        private async Task<String> SaveFile(IFormFile file)
        {
            ...
            await _storagesService.SaveFileAsync(file.OpenReadStream(), fileName);
